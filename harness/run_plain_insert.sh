#!/usr/bin/env bash
# Corrected RUN 1: fair raw insert into bench_items_plain (NO realtime), both framings, N=5.
set -uo pipefail
cd ~/HyperStack
source bench/config.sh

D=/usr/local/bin/docker
RUNID="$(date +%Y%m%d_%H%M%S)_plainins"
RAW="${BENCH_RAW_DIR}"
NET="${DOCKER_NETWORK}"
...
run_one() {
  ...
  $D run --rm \
    ...
    "${K6_IMAGE}" run --quiet \
      ...
      "/scenarios/rest-insert-plain.js" >/dev/null 2>&1 || ec=$?
  if [[ "$ec" -eq 99 ]]; then
...
for framing in a b; do
  ...
  for run_n in 1 2 3 4 5; do
    run_one "$framing" "hyperstack" "${RAW}/k6-${framing}-rest-insert-plain-hyperstack-${RUNID}-run${run_n}.json" \
      "$hs_url" "$hs_jwt" "$hs_uid" "$sb_url" "$sb_jwt" "$sb_uid"
    run_one "$framing" "supabase" "${RAW}/k6-${framing}-rest-insert-plain-supabase-${RUNID}-run${run_n}.json" \
      "$hs_url" "$hs_jwt" "$hs_uid" "$sb_url" "$sb_jwt" "$sb_uid"
  done
done
